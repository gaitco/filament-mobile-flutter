...
void main() {
  ...
  test(
    'exercises every published filter kind against a live host',
    () async {
      ...
      final rawResource = rawResources
      ...
      final statusOptions =
          (rawFilter('status')['config']['options'] as List)
              .cast<Map<String, dynamic>>()
              .map((o) => o['value'])
              .toSet();
      ...
      expect(
        onlyTrashed.records,
        isNotEmpty,
        reason: 'seed at least one soft-deleted $key row on the host before '
            'running this test',
      );
      expect(
      ...
      expect(
        ...
        isFalse,
        reason: 'a filter[trashed]=0 record must not appear in the default '
